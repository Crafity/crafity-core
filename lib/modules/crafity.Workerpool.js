...
/*globals exports:true, module, require, Workerpool, console, setTimeout, clearTimeout */
/*!
 * crafity.Workerpool
...
try {
  ...
  Domain.createDomain = function () { return new Domain(); };
}
  ...
  function WorkerContext() {
    ...
    this.run = function (work, callback) {
      ...
      function onCompleteHandler(ex, result) {
        if (self.debug) { console.log(work.report ? "C O M P L E T E  " + work.report.name : ""); }
        if (self.isCompleted) {
      ...
      try {
        ...
        if (self.debug) { console.log("Sychronous part is done", work.report.name, innerOnCompleted.listenerCount); }
        if (!self.isAsync) {
          onCompleteHandler(null, output);
        } else {
          timeoutId = setTimeout(function () {
            timeoutId = null;
            if (self.debug) { console.log("TIMEEEE OUTTTT....", work.report.name, innerOnCompleted.listenerCount); }
            innerOnCompleted.raise(new Error("Work item exceeded the specified time out"), null);
          }, self.asyncTimeout);
          if (self.debug) { console.log("Time out is set", self.asyncTimeout, timeoutId.toString(), work.report.name, innerOnCompleted.listenerCount); }
        }
      } catch (ex) {
